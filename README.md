...
Download the data for the omim_ingest transform:

```bash
poetry run omim_ingest download
```

To run the Koza transform for omim-ingest:

```bash
poetry run omim_ingest transform
```

To see available options:

```bash
poetry run omim_ingest download --help
# or
poetry run omim_ingest transform --help
```
